...
local function attach_client(client)
  ...
  coro_debugger = corocreate(debugger_loop)
  debug.sethook(debug_hook, HOOKMASK)
  seen_hook = nil -- reset in case the last attach attempt was refused
  step_into = true -- start with step command
  return true
end

-- Starts a non-blocking debugger listener.
-- Use poll() from the main loop to accept and attach when a client arrives.
local function listen_background(host, port)
  host = host or "*"
  port = port or mobdebug.port

  if passive_server then return true end

  local srv, err = socket.bind(host, port)
  if not srv then return nil, err end

  if srv.settimeout then srv:settimeout(0) end
  passive_server = srv
  return true
end

-- Polls a previously started background listener and attaches if a client connected.
-- Returns true when attached, false when no client is pending, nil on socket error.
local function poll()
  if isrunning() then return true end
  if not passive_server then return false end

  local client, err = passive_server:accept()
  if not client then
  ...
  print(("[mobdebug] client connected from %s:%s"):format(tostring(peer_ip), tostring(peer_port)))

  local ok, attach_err = attach_client(client)
  if not ok then return nil, attach_err end

  print("[mobdebug] debugger attached")
  return true
end
...
function listen(host, port)
  ...
  print(("[mobdebug] listening on %s:%s"):format(host, tostring(port)))
  print("Run the program you wish to debug")

  local server = socket.bind(host, port)
  print("[mobdebug] waiting for debugger client...")
  local client = server:accept()
  local peer_ip, peer_port = client:getpeername()
  print(("[mobdebug] client connected from %s:%s"):format(tostring(peer_ip), tostring(peer_port)))

  print("[mobdebug] handshake: sending STEP")
  client:send("STEP\n")
  local step_ack = client:receive("*l")
  print(("[mobdebug] handshake: STEP ack = %s"):format(tostring(step_ack)))

  local breakpoint = client:receive("*l")
  print(("[mobdebug] handshake: breakpoint packet = %s"):format(tostring(breakpoint)))
  ...
  while true do
    ...
    print(("[mobdebug] console command: %s"):format(tostring(cmd)))
    local file, _, err = handle(cmd, client, {
      verbose = function(msg)
        print("[mobdebug] " .. tostring(msg))
      end
    })
    print(("[mobdebug] command result: file=%s err=%s"):format(tostring(file), tostring(err)))
...
mobdebug.listen_background = listen_background
mobdebug.poll = poll
mobdebug.loop = loop
